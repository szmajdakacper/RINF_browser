...
def _find_in_xml(path: str, tag: str, predicate_xpath: str) -> str | None:
    """Wyszukuje pierwszy element pasujacy do XPath i zwraca jego pretty-printed XML."""
    if not path:
        return None
    try:
        for _, elem in etree.iterparse(path, events=("end",), tag=tag):
            try:
                if elem.xpath(predicate_xpath):
...
def render():
    ...
    if not ds.source_path:
        st.warning("Plik byl wczytany z uploadu — brak sciezki na dysku do XPath. Wczytaj plik przez sciezke.")
        return
    ...
    if mode == "OperationalPoint":
        ...
        if st.button("Pokaz XML"):
            xml = _find_in_xml(ds.source_path, "OperationalPoint", f"./UniqueOPID[@Value='{opid}']")
            if xml:
        ...
        if st.button("Pokaz XML"):
            ...
            xpath = (
                f"./SOLLineIdentification[@Value='{sol.line_id}']/.."
                f"[./SOLOPStart[@Value='{sol.op_start_id}']]"
                f"[./SOLOPEnd[@Value='{sol.op_end_id}']]"
            )
            # Prostsze podejscie: szukaj po start+end
            target = None
